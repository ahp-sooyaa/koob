	...
	<div class="w-full">
		...
		>
			...
			<textarea
				...
				cols="35"
				rows="5"
				class="rounded-2xl text-sm p-5 mt-4 mb-2 block border border-gray-300 focus:border-gray-500 focus:ring focus:ring-gray-300"
		...
		<div
			v-else
			class="flex justify-center space-x-2 rounded-md p-3 text-gray-700 text-sm tracking-widest"
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
			...
			<span>
				Please <Link
					:href="route('login')"
					class="underline hover:text-gray-900 font-semibold"
				>login</Link> first to give review & rating.
			</span>
		</div>
		<div
		...
		>
			<div
				...
				:key="index"
				class="w-full bg-white p-5 rounded-lg border"
			>
				<div
					v-if="! review.approved_at || review.id === authUserReview?.id"
					class="flex justify-between px-5 py-2 text-sm font-light mb-5 -mx-5 -mt-5 border-b rounded-t-lg bg-gray-50"
				>
					<p
						v-if="! review.approved_at"
					...
					<div class="flex-1">
						<div class="flex justify-between">
							<div>
								...
								<span class="hidden md:inline text-xs text-gray-500">reviewed on {{ formatDate(review.created_at) }}</span>
								<span class="hidden md:inline text-gray-500 mx-1">&CenterDot;</span>
								<span class="hidden md:inline text-xs text-gray-700">verified purchase</span>
							</div>
							<div class="flex md:mb-5 text-red-500">
								<svg
									v-for="(rating, index) in parseInt(review.rating)"
									:key="index"
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 20 20"
									fill="currentColor"
								...
								<svg
									v-for="(emptyStar, index) in parseInt(5 - review.rating)"
									:key="index"
									xmlns="http://www.w3.org/2000/svg"
									fill="none"
									viewBox="0 0 24 24"
